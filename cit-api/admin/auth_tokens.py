...
def get_access_token(request):
    ...
    if existing_access_token and existing_access_token_expiry:
        expires_in = (
            dateutil.parser.parse(existing_access_token_expiry)
            - datetime.datetime.now()
        )
        if expires_in > datetime.timedelta(minutes=30):
            print(
                "returning existing access token which expires at {}".format(expires_in)
            )
            return JsonResponse({"access_token": existing_access_token})
    ...
    try:
        ...
        response = clientapp.acquire_token_silent(
            scopes=[os.environ.get('SCOPE')], account=None
        )
        if not response:
