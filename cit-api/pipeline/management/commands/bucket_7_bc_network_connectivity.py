from pipeline.importers.bucket7.bucket7_bc_network_connectivity import (
    import_data_sources,
)
...
class Command(BaseImportCommand):
    ...
    def handle(self, *args, **options):
        ...
        import_data_sources()
        # Ensure that the data sources are updated
        data_resources = DataSource.objects.filter(name__in=['bc_network_connectivity'])

        for resource in data_resources:
            print(f'Importing {resource.display_name}...')
            import_wms_resource(resource)

        print("Import process for BC Network connectivity completed!")
