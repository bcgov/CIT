...
from pipeline.importers.bucket2_regional_districts import import_data_sources
from pipeline.importers.databc_resource import import_wms_resource
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if settings.ENV_LEVEL in ["test", "prod"]:
            ...
            for folder in self.SUB_FOLDERS:
                folder_path = os.path.join(
                    settings.AZURE_BLOB_STORAGE_LOCAL_PATH, folder
                )
                print(folder_path)
            ...
            for blob in container_client.list_blobs():
                ...
                download_file_path = os.path.join(
                    settings.AZURE_BLOB_STORAGE_LOCAL_PATH, blob.name
                )
        ...
        import_data_sources()
        # Ensure that the data sources are updated
        data_resources = DataSource.objects.filter(name__in=["regional_districts"])
