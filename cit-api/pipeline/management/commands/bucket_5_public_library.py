from pipeline.importers.bucket5.bucket5_public_library import import_data_sources
...
class Command(BaseImportCommand):
    ...
    def handle(self, *args, **options):
        ...
        import_data_sources()
        # Ensure that the data sources are updated
        data_resources = DataSource.objects.filter(name__in=['public_library'])

        for resource in data_resources:
            print(f'Importing {resource.display_name}...')
            import_wms_resource(resource)

        print("Import process for bucket5_public_library completed!")
