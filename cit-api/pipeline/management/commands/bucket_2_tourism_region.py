from pipeline.importers.bucket2.bucket2_tourism_region import import_data_sources
...
class Command(BaseImportCommand):
    ...
    def handle(self, *args, **options):
        ...
        import_data_sources()
        # Ensure that the data sources are updated
        data_resources = DataSource.objects.filter(name__in=['tourism_regions'])

        for resource in data_resources:
            import_wms_resource(resource)
        print("Import process for Tourism Regions completed!")
