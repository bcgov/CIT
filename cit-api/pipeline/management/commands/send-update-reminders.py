...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        stale_opportunities = Opportunity.objects.filter(
            date_updated__lte=stale_time, deleted=False, approval_status="PUBL"
        ).exclude(user_id__email__in=['', 'Unknown'])
        print(
            "Total stale opportunities to be notified:"
            + str(stale_opportunities.count())
        )
        ...
        for email in email_opportunity_mapping:
            ...
            if email_result.status_code == 201:
                print(
                    "Successfully sent notification for opportunity with ID "
                    + str(
                        [
                            mapped_opportunity.id
                            for mapped_opportunity in email_opportunity_mapping[email]
                        ]
                    )
                )
            else:
                ...
                print(
                    "Unable to send notification for opportunity with ID "
                    + str(
                        [
                            mapped_opportunity.id
                            for mapped_opportunity in email_opportunity_mapping[email]
...
def construct_email_body(opportunity_list):
    ...
    for opportunity in opportunity_list:
        ...
        opportunity_links = opportunity_links + build_individual_opportunity_link(
            opportunity
        )
        opportunity_links = opportunity_links + "</p>"
    ...
    email_body = (
        email_body
        + "<p>To manage these listings, please log in to the <a href=\""
        + build_full_dashboard_link()
        + "\">Community Investment Opportunities Tool</a>.</p>"
    )
    email_body = (
        email_body
        + "<p>If the opportunity is still active, click on 'Edit Listing' to review and resubmit your listing.</p>"
    )
    email_body = (
        email_body + "<p>If the opportunity has been sold, please select 'Close'.</p>"
    )
    email_body = (
        email_body
        + "<p>If you wish to remove the opportunity from the site, please select 'Delete'.</p>"
    )
    email_body = (
        email_body
        + "<p>Thank you for your cooperation in keeping the Community Investment Opportunities Tool up-to-date and valuable for investors.</p>"
    )
...
def build_full_dashboard_link():
    return (
        os.environ.get("EMAIL_OPPORTUNITY_LINK_HOST")
        + "/investmentopportunities/dashboard"
    )


def build_individual_opportunity_link(opportunity):
    opportunity_view_link = (
        os.environ.get("EMAIL_OPPORTUNITY_LINK_HOST")
        + "/investmentopportunities/view/"
        + to_kebab_case(opportunity.opportunity_name)
        + "-"
        + str(opportunity.id)
    )
...
def to_kebab_case(opportunity_title):
    title_regex_match_list = re.findall(
