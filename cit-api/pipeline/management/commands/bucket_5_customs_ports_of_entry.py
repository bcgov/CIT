from pipeline.importers.bucket5.bucket5_customs_ports_of_entry import (
    import_data_sources,
)
...
class Command(BaseImportCommand):
    ...
    def handle(self, *args, **options):
        ...
        import_data_sources()
        # Ensure that the data sources are updated
        data_resources = DataSource.objects.filter(name__in=['customs_ports_of_entry'])

        for resource in data_resources:
            print(f'Importing {resource.display_name}...')
            import_wms_resource(resource)

        print("Import process for bucket5_customs_ports_of_entry completed!")
