...
class CommunitySerializer(serializers.ModelSerializer):
    ...
    wildfire_zone = serializers.SlugRelatedField(
        read_only=True, slug_field='risk_class'
    )
    tsunami_zone = serializers.SlugRelatedField(read_only=True, slug_field='zone_class')
    community_type = serializers.CharField(
        source='get_display_community_type', read_only=True
    )
...
class CommunityCSVSerializer(serializers.ModelSerializer):
    queryset = (
        ...
        .select_related('hexuid', 'hexuid__service__isp')
        .prefetch_related('hexuid__service')
    )
    wildfire_zone = serializers.SlugRelatedField(
        read_only=True, slug_field='risk_class'
    )
    ...
    census_subdivision_median_total_household_income_one_person = serializers.CharField(
        read_only=True,
        source='census_subdivision.median_total_household_income_one_person',
    )
    census_subdivision_median_total_household_income_two_or_more_person = serializers.CharField(
        read_only=True,
        source='census_subdivision.median_total_household_income_two_or_more_person',
    )
    ...
    census_subdivision_households_owner_pct_spending_30_pct_income = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.households_owner_pct_spending_30_pct_income',
        )
    )
    ...
    census_subdivision_households_owner_count_spending_30_pct_income = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.households_owner_count_spending_30_pct_income',
        )
    )
    census_subdivision_households_owner_median_monthly_shelter_costs = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.households_owner_median_monthly_shelter_costs',
        )
    )
    census_subdivision_households_owner_avg_monthly_shelter_costs = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.households_owner_avg_monthly_shelter_costs',
        )
    )
    census_subdivision_households_owner_median_dwelling_value = serializers.CharField(
        read_only=True,
        source='census_subdivision.households_owner_median_dwelling_value',
    )
    ...
    census_subdivision_households_tenant_pct_subsidized_housing = serializers.CharField(
        read_only=True,
        source='census_subdivision.households_tenant_pct_subsidized_housing',
    )
    census_subdivision_households_tenant_pct_spending_30_pct_income = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.households_tenant_pct_spending_30_pct_income',
        )
    )
    ...
    census_subdivision_households_tenant_count_subsidized_housing = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.households_tenant_count_subsidized_housing',
        )
    )
    census_subdivision_households_tenant_count_spending_30_pct_income = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.households_tenant_count_spending_30_pct_income',
        )
    )
    census_subdivision_households_tenant_median_shelter_cost = serializers.CharField(
        read_only=True,
        source='census_subdivision.households_tenant_median_shelter_cost',
    )
    ...
    census_subdivision_edu_field_personal_protective_transportation = (
        serializers.CharField(
            read_only=True,
            source='census_subdivision.edu_field_personal_protective_transportation',
        )
    )
