...
class ProximityView(APIView):
    ...
    def get(self, request, format=None):
        ...
        airport = None
        airport_check = (
            Location.objects.annotate(distance=Distance("point", point))
            .filter(point__distance_lte=(point, D(km=100)), location_type="airports")
            .order_by('distance')[:1]
        )
        if airport_check:
            airport = json.loads(
        ...
        deep_port_check = (
            ...
            .filter(
                ...
                location_type="port_and_terminal",
            )
            .order_by('distance')[:1]
        )
        if deep_port_check:
        ...
        customs_port = None
        customs_port_check = (
            CustomsPortOfEntry.objects.annotate(distance=Distance("point", point))
            .filter(point__distance_lte=(point, D(km=100)))
            .order_by('distance')[:1]
        )
        if customs_port_check:
            customs_port = json.loads(
                serializers.serialize(
                    'geojson', customs_port_check, geometry_field=point
                )
            )
            customs_port['customs_port_distance'] = (
                customs_port_check.first().distance.km
            )
        ...
        post_secondary_check = (
            ...
            .filter(
                point__distance_lte=(point, D(km=100)),
                location_type="post_secondary_institutions",
            )
            .order_by('distance')[:1]
        )
        if post_secondary_check:
            post_secondary = json.loads(
                serializers.serialize(
                    'geojson', post_secondary_check, geometry_field=point
                )
            )
            post_secondary['post_secondary_distance'] = (
                post_secondary_check.first().distance.km
            )

        railway = None
        railway_check = (
            Railway.objects.annotate(distance=Distance("geom", point))
            .filter(geom__distance_lte=(point, D(km=100)))
            .order_by('distance')[:1]
        )
        if railway_check:
            railway = json.loads(
        ...
        highway = None
        highway_check = (
            RoadsAndHighways.objects.annotate(distance=Distance("geom", point))
            .filter(geom__distance_lte=(point, D(km=100)), feature_type="Road")
            .order_by('distance')[:1]
        )
        if highway_check:
            highway = json.loads(
        ...
        research_centre_check = (
            Location.objects.annotate(distance=Distance("point", point))
            .filter(
                point__distance_lte=(point, D(km=100)), location_type="research_centres"
            )
            .order_by('distance')[:1]
        )
        if research_centre_check:
            research_centre = json.loads(
                serializers.serialize(
                    'geojson', research_centre_check, geometry_field=point
                )
            )
            research_centre['research_centre_distance'] = (
                research_centre_check.first().distance.km
            )
        ...
        if community_check:
            ...
            transmission['distance'] = (
                community_check.first().nearest_transmission_distance
                + community['community_distance']
            )
            properties = community['features'][0]['properties']
            network_percents = [
                properties['percent_5_1'],
                properties['percent_10_2'],
                properties['percent_25_5'],
            ...
            for network in network_percents:
                if (
                    last_network is not None
                    and network is not None
                    and abs(last_network - network) < 0.1
                ):
        ...
        nearest_fire_station_check = (
            ...
            .filter(
                point__distance_lte=(point, D(km=100)),
                firstresponder__keywords__contains="fire",
            )
            .order_by('distance')[:1]
        )
        if nearest_fire_station_check:
            nearest_fire_station = json.loads(
                serializers.serialize(
                    'geojson', nearest_fire_station_check, geometry_field=point
                )
            )
            nearest_fire_station['distance'] = (
                nearest_fire_station_check.first().distance.km
            )
        ...
        nearest_police_station_check = (
            ...
            .filter(
                point__distance_lte=(point, D(km=100)),
                firstresponder__keywords__contains="police",
            )
            .order_by('distance')[:1]
        )
        if nearest_police_station_check:
            nearest_police_station = json.loads(
                serializers.serialize(
                    'geojson', nearest_police_station_check, geometry_field=point
                )
            )
            nearest_police_station['distance'] = (
                nearest_police_station_check.first().distance.km
            )
        ...
        nearest_ambulance_station_check = (
            ...
            .filter(
                point__distance_lte=(point, D(km=100)),
                firstresponder__keywords__contains="ambulance",
            )
            .order_by('distance')[:1]
        )
        if nearest_ambulance_station_check:
            nearest_ambulance_station = json.loads(
                serializers.serialize(
                    'geojson', nearest_ambulance_station_check, geometry_field=point
                )
            )
            nearest_ambulance_station['distance'] = (
                nearest_ambulance_station_check.first().distance.km
            )
        ...
        nearest_coast_guard_station_check = (
            ...
            .filter(
                point__distance_lte=(point, D(km=100)),
                firstresponder__keywords__contains="coastguard",
            )
            .order_by('distance')[:1]
        )
        if nearest_coast_guard_station_check:
            nearest_coast_guard_station = json.loads(
                serializers.serialize(
                    'geojson', nearest_coast_guard_station_check, geometry_field=point
                )
            )
            nearest_coast_guard_station['distance'] = (
                nearest_coast_guard_station_check.first().distance.km
            )

        nearest_health_center = None
        nearest_health_center_check = (
            Location.objects.annotate(distance=Distance("point", point))
            .filter(point__distance_lte=(point, D(km=100)), location_type='hospitals')
            .order_by('distance')[:1]
        )
        if nearest_health_center_check:
            nearest_health_center = json.loads(
                serializers.serialize(
                    'geojson', nearest_health_center_check, geometry_field=point
                )
            )
            nearest_health_center['distance'] = (
                nearest_health_center_check.first().distance.km
            )

        network_at_road = 'Unknown network'
        network_at_road_check = (
            Road.objects.annotate(distance=Distance("geom", point))
            .filter(geom__distance_lte=(point, D(m=150)))
            .order_by('distance')[:1]
        )
        ...
        municipalities_check = (
            Municipality.objects.annotate(distance=Distance("geom", point))
            .filter(geom__distance_lte=(point, D(km=100)))
            .order_by('distance')[:5]
        )
        if municipalities_check:
            municipalities = json.loads(
                serializers.serialize(
                    'geojson', municipalities_check, geometry_field=point
                )
            )
            ...
            while index < len(municipalities_check):
                ...
                municipalities['features'][index]['properties']['distance'] = (
                    municipalities_check[index].distance.km
                )
                municipalities['features'][index]['properties']['population'] = 0
                community_queryset = Community.objects.filter(
                    municipality_id=municipality_id
                ).annotate(count=Sum(F('census_subdivision__pop_total_2016')))
                ...
                if len(community_queryset):
                    municipalities['features'][index]['properties'][
                        'community_id'
                    ] = community_queryset.values()[0]['id']
                    municipalities['features'][index]['properties'][
                        'population'
                    ] = community_queryset.values()[0]['count']
                else:
                    total_pop = Community.objects.raw(
                        f'select pm.id, pcpdcas.* from pipeline_municipality pm, pipeline_cen_prof_detailed_csd_attrs_sp pcpdcas where pm.id = {municipality_id} and st_intersects(pm.geom, pcpdcas.geom) and (st_area(st_intersection(pm.geom, pcpdcas.geom))/st_area(pm.geom)) > .55;'
                    )[0].pop_total_2016
                    municipalities['features'][index]['properties'][
                        'population'
                    ] = total_pop
        ...
        if first_nation_community_check:
            first_nation_communities = json.loads(
                serializers.serialize(
                    'geojson', first_nation_community_check, geometry_field=point
                )
            )
            # Add the missing annotated distance value to each geojson feature
            index = 0
            while index < len(first_nation_community_check):
                first_nation_communities['features'][index]['properties'][
                    'distance'
                ] = first_nation_community_check[index].distance.km
                index += 1
