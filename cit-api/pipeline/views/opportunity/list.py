...
class OpportunitiesList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if regional_district is not None:
            regional_district_models = RegionalDistrict.objects.get(
                id=regional_district
            )
            if regional_district_models is not None:
                queryset = queryset.filter(
                    geo_position__intersects=regional_district_models.geom
                )
        ...
        post_secondary_within_100km = self.request.query_params.get(
            'post_secondary_within_100km', None
        )
        if post_secondary_within_100km == 'Y':
            queryset = queryset.filter(
                nearest_post_secondary__location_distance__lte=100
            )

        research_centre_within_100km = self.request.query_params.get(
            'research_centre_within_100km', None
        )
        if research_centre_within_100km == 'Y':
            queryset = queryset.filter(
                nearest_research_centre__research_centre_distance__lte=100
            )
        ...
        community_distance = float(
            self.request.query_params.get('community_distance', INVALID_INT)
        )
        if community_distance >= MIN_DISTANCE and community_id >= MIN_TABLE_ID:
            ...
            if community_model is not None:
                queryset = queryset.filter(
                    geo_position__distance_lte=(
                        community_model.point,
                        D(km=community_distance),
                    )
                )

        parcel_size_min = float(
            self.request.query_params.get('parcel_size_min', INVALID_INT)
        )
        parcel_size_max = float(
            self.request.query_params.get('parcel_size_max', INVALID_INT)
        )
        ...
        if power_transmission_lines_min >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_transmission_line__gte=power_transmission_lines_min
            )
        if power_transmission_lines_max >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_transmission_line__lte=power_transmission_lines_max
            )

        air_service_min = float(
            self.request.query_params.get('air_service_min', INVALID_INT)
        )
        air_service_max = float(
            self.request.query_params.get('air_service_max', INVALID_INT)
        )
        if air_service_min >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_airport__airport_distance__gte=air_service_min
            )
        if air_service_max >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_airport__airport_distance__lte=air_service_max
            )
        ...
        if rail_connections_min >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_railway__railway_distance__gte=rail_connections_min
            )
        if rail_connections_max >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_railway__railway_distance__lte=rail_connections_max
            )
        ...
        if deep_water_port_min >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_port__port_distance__gte=deep_water_port_min
            )
        if deep_water_port_max >= MIN_SIZE:
            queryset = queryset.filter(
                nearest_port__port_distance__lte=deep_water_port_max
            )
        ...
        community_population_distance_min = float(
            self.request.query_params.get(
                'community_population_distance_min', INVALID_INT
            )
        )
        community_population_distance_max = float(
            self.request.query_params.get(
                'community_population_distance_max', INVALID_INT
            )
        )
        ...
        if proximity_community_id >= MIN_TABLE_ID and (
            community_population_distance_min >= MIN_SIZE
        ...
        if proximity_population >= MIN_TABLE_ID and (
            community_population_distance_min >= MIN_SIZE
            or community_population_distance_max >= MIN_SIZE
        ):
        ...
        return (
            queryset.select_related(
                'nearest_port__port_id',
                'nearest_airport__airport_id',
                'nearest_research_centre',
                'nearest_customs_port_of_entry',
                'nearest_coast_guard_station',
                'nearest_community__community_id',
                'nearest_ambulance_station',
                'nearest_police_station',
                'nearest_fire_station',
                'nearest_health_center',
                'nearest_railway__railway_id',
                'nearest_highway__highway_id',
                'nearest_river',
                'nearest_lake',
            ...
            .prefetch_related(
                'nearest_first_nations__reserve_id',
    ...
    def filter_opportunities_by_distance_from_community(
        self,
        queryset,
        community_distance_min,
    ...
    def filter_opportunities_by_distance_from_population(
        self, queryset, population_distance_min, population_distance_max, population
    ):
