...
def serialize_regional_district_fields(regional_district):
    ...
    census_aggregate_values = census_subdivisions.aggregate(
        ...
        Sum("edu_field_total"),
        Sum("edu_2"),
        population_density_unweighted=Sum(F("population_density_per_sq_km") * F("area"),
                                          output_field=FloatField()),
        population_age_unweighted=Sum(F("population_avg_age") * F("population"),
                                      output_field=FloatField()),
        employment_rate_unweighted=Sum(F("employment_rate") * F("population"),
                                       output_field=FloatField()))

    population_density = census_aggregate_values[
        "population_density_unweighted"] / census_aggregate_values["area__sum"]
    average_age = census_aggregate_values["population_age_unweighted"] / census_aggregate_values[
        "population__sum"]
    employment_rate = (census_aggregate_values["employment_rate_unweighted"] /
                       census_aggregate_values["population__sum"])
    percent_post_secondary = (census_aggregate_values["edu_2__sum"] /
                              census_aggregate_values["edu_field_total__sum"]) * 100
...
def get_hidden_explore_report_pages(communities):
    ...
    if all(community.census_subdivision.get_percentage_of_null_fields() > 0.25
