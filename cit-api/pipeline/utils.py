...
from django.db.models import Avg, F, FloatField, Q, Sum
...
def serialize_community_detail_fields(obj):
    return [
        ...
        },
        {
            ...
            and (str(obj.census_subdivision.population_percentage_change) + "%"),
            "metadata": {
...
def serialize_regional_district_fields(regional_district):
    ...
    census_aggregate_values = census_subdivisions.aggregate(
        ...
        Sum("edu_field_total"),
        Sum("edu_2"),
        population_density_unweighted=Sum(
            F("population_density_per_sq_km") * F("area"), output_field=FloatField()
        ),
        population_age_unweighted=Sum(
            F("population_avg_age") * F("population"), output_field=FloatField()
        ),
        employment_rate_unweighted=Sum(
            F("employment_rate") * F("population"), output_field=FloatField()
        ),
    )

    population_density = (
        census_aggregate_values["population_density_unweighted"]
        / census_aggregate_values["area__sum"]
    )
    average_age = (
        census_aggregate_values["population_age_unweighted"]
        / census_aggregate_values["population__sum"]
    )
    employment_rate = (
        census_aggregate_values["employment_rate_unweighted"]
        / census_aggregate_values["population__sum"]
    )
    percent_post_secondary = (
        census_aggregate_values["edu_2__sum"]
        / census_aggregate_values["edu_field_total__sum"]
    ) * 100
        ...
        for location_asset in location_assets:
            temp = {
                ...
                "distance": (
                    ...
                    if location_asset.driving_distance is not None
                ...
                **{
                    ...
                    for field in get_fields_for_location_type(
...
def get_location_assets_for_community(model_class, community):
    from pipeline.models.location_assets import Project, School

    if model_class == School:
        school_districts = community.schooldistrict_set.all()
        return School.objects.filter(
            Q(distances__community=community)
            & Q(school_district__in=school_districts)
            & (
                Q(distances__driving_distance__lte=50)
                | (
                    Q(distances__driving_distance__isnull=True)
                    & Q(distances__distance__lte=50)
                )
            )
        ).annotate(
    ...
    if model_class == Project:
        return (
            ...
            .order_by('project_id', '-source_date')
            .distinct('project_id')
        )
    else:
...
def communities_advanced_search(query_params):
    ...
    for query_filter in filters:
        ...
        if query_filter["field"] == "location":
            ...
            query += query_filter["field"]
            query_filter["value"] = [
                True if value == "true" else False for value in query_filter["value"]
            ]
        elif query_filter["field"] == "community_type":
            query += query_filter["field"]
            if "Indigenous" in query_filter["value"]:
                query_filter["value"].remove("Indigenous")
                query_filter["value"].extend(
                    ["Urban First Nations Reserve", "Rural First Nations Reserve"]
                )
    ...
    if len(overall_query) > 0:
        communities = Community.objects.filter(
            Q(reduce(and_, (q for q in overall_query)))
        )
    else:
...
def _handle_location_filter(query_filter):
    ...
    location_type_query = Q(
        **{"distances__location__location_type": query_filter["location_type"]}
    )
    distance_query = None
    query = ""
...
def serialize_data_source(name, datasource_info):
    from pipeline.constants import DATABC_PERMALINK_URL, SOURCE_DATABC, SOURCE_INTERNAL
    ...
    if datasource_info["source"] == SOURCE_INTERNAL:
        ...
        source_url = DATABC_PERMALINK_URL.format(
            permalink_id=datasource_info["permalink_id"]
        )
...
def get_communities_with_insufficient_data(communities):
    return [
        ...
        if community.census_subdivision.get_percentage_of_null_fields() > 0.25
    ]
