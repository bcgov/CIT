...
def remove_status_new(apps, schema_editor):
    ...
    new_opportunities = opportunity.objects.filter(
        approval_status_id__in=['NEW', 'NWED']
    ).update(approval_status_id='PEND')

    # remove NEW & NWED from pipeline_approvalstatus
    old_approval_statuses = approval_status.objects.filter(
        status_code__in=['NEW', 'NWED']
    ).delete()
...
def add_status_new(apps, schema_editor):
    ...
    approval_status.objects.update_or_create(
        status_name='New',
        status_description='Opportunity has just been submitted',
        status_code='NEW',
    )
    approval_status.objects.update_or_create(
        status_name='New - Edited',
        status_description='Opportunity has been udpate by EDO for further review',
        status_code='NWED',
    )
